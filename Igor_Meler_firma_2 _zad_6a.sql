...
CREATE TABLE ksiegowosc.wynagrodzenie (
	...
	INSERT INTO ksiegowosc.pensje VALUES (1, 'analityk', 3100);
	...
	INSERT INTO ksiegowosc.wynagrodzenie VALUES (7, '2022-04-11', 1, 1, 3, 1); 
	...
	ALTER TABLE ksiegowosc.godziny DROP CONSTRAINT FK__godziny__id_prac__6A30C649;
	ALTER TABLE ksiegowosc.wynagrodzenie DROP CONSTRAINT FK__wynagrodz__id_pr__70DDC3D8;
	...
	SELECT TOP 1 id_pracownika, imie, UPPER(nazwisko) as nazwisko, adres, telefon FROM ksiegowosc.pracownicy
	...
	CONVERT(VARCHAR(32), HashBytes('MD5', CONVERT(VARCHAR(32),pensje.kwota)),2) AS pensja
	FROM ((ksiegowosc.pracownicy 
	INNER JOIN ksiegowosc.wynagrodzenie ON wynagrodzenie.id_pracownika = pracownicy.id_pracownika)
	INNER JOIN ksiegowosc.pensje ON pensje.id_pensji = wynagrodzenie.id_pensji);


	SELECT pracownicy.id_pracownika, pensje.kwota AS pensja, premie.kwota AS premia
	FROM (((ksiegowosc.pracownicy
	...
	SELECT pracownicy.imie +' ' + pracownicy.nazwisko + ', w dniu ' + CONVERT(VARCHAR(32), godziny.data) + ' otrzyma³ pensjê ca³kowit¹ na kwotê ' +
		CONVERT(VARCHAR(32),pensje.kwota + premie.kwota) + ', gdzie wynagrodzenie zasadnicze wynosi³o: ' + CONVERT(VARCHAR(32), pensje.kwota) +
		', premia:' + CONVERT(VARCHAR(32), premie.kwota) + ', nadgodziny: ' + CONVERT(VARCHAR(32), premie.kwota + 100) AS RAPORT
		FROM ((((ksiegowosc.pracownicy
		INNER JOIN ksiegowosc.wynagrodzenie ON wynagrodzenie.id_pracownika = pracownicy.id_pracownika)
		INNER JOIN ksiegowosc.godziny ON godziny.id_godziny = wynagrodzenie.id__godziny)
		INNER JOIN ksiegowosc.pensje ON pensje.id_pensji = wynagrodzenie.id_pensji)
